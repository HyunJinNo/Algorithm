...
const path = process.platform === "linux" ? "/dev/stdin" : "./src/input.txt";
const input = require("fs").readFileSync(path).toString().split("\n");

// N: 학생의 수, 1 <= N <= 32_000
// M: 키를 비교한 횟수, 1 <= M <= 100_000
const [N, M] = input[0].split(" ").map(Number);
const inDegree = new Int32Array(N + 1).fill(0); // 진입 차수
...
for (let i = 1; i <= N; i++) {
  // 진입 차수가 0인 노드들 큐에 삽입한다.
  if (inDegree[i] === 0) {
    queue.addLast(i);
  }
}

// 위상 정렬이 완전히 수행된다면 정확히 N개의 노드를 방문한다.
